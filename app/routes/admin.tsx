...
export default function Admin() {
  ...
  const fetchMemoryStats = async () => {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        console.log('Memory stats response:', data); // Debug log
        setMemoryStats(data.memory);
      } else {
        console.error('Error fetching memory stats:', response.status, response.statusText);
      }
    } catch (error) {
                  ...
                  <div className="space-y-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="bg-gray-700 rounded p-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-yellow-400 font-bold">{memoryStats?.userConnections || 0}</span>
                          </div>
                          <div className="w-full bg-gray-600 rounded-full h-2">
                            <div 
                              className="bg-yellow-400 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${Math.min((memoryStats?.userConnections || 0) / 1000 * 100, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-700 rounded p-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-blue-400 font-bold">{memoryStats?.games || 0}</span>
                          </div>
                          <div className="w-full bg-gray-600 rounded-full h-2">
                            <div 
                              className="bg-blue-400 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${Math.min((memoryStats?.games || 0) / 100 * 100, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-700 rounded p-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-green-400 font-bold">{memoryStats?.activeGames || 0}</span>
                          </div>
                          <div className="w-full bg-gray-600 rounded-full h-2">
                            <div 
                              className="bg-green-400 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${Math.min((memoryStats?.activeGames || 0) / 50 * 100, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-700 rounded p-4">
                          <div className="flex justify-between items-center mb-2">
                            ...
                            <span className="text-purple-400 font-bold">{memoryStats.games}</span>
                          </div>
                          <div className="w-full bg-gray-600 rounded-full h-2">
                            <div 
                              className="bg-purple-400 h-2 rounded-full transition-all duration-300"
                              style={{ width: `${Math.min((memoryStats.games / 100) * 100, 100)}%` }}
                            ></div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="bg-gray-800 rounded-lg p-6">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className={`rounded p-4 text-center ${
                          (memoryStats?.userConnections || 0) < 500 ? 'bg-green-900/30 border border-green-500/30' :
                          (memoryStats?.userConnections || 0) < 1000 ? 'bg-yellow-900/30 border border-yellow-500/30' :
                          'bg-red-900/30 border border-red-500/30'
                        }`}>
                          <div className={`text-2xl font-bold ${
                            (memoryStats?.userConnections || 0) < 500 ? 'text-green-400' :
                            (memoryStats?.userConnections || 0) < 1000 ? 'text-yellow-400' :
                            'text-red-400'
                          }`}>
                            {memoryStats?.userConnections || 0}
                          </div>
